$(function () {
    // Add Menu
    $("#add").click(function () {
        $.get("/VehicleReserves/FormAddReservesStatus", function (ReturnForm) {
            ...
            $("#submit").click(function () {
                ...
                if ($('#FormAddStatus').data("bootstrapValidator").isValid() == true) {
                    ...
                    {
                        type: "POST",
                        url: "/VehicleReserves/AddReservesStatus",
                        contentType: false,
                        processData: false,
                        data: Data,
                        success: function (response) {
                            if (response.valid == true) {
                                ...
                                setTimeout(function () {
                                    window.location.href = "/VehicleReserves/ReservesStatus";
                                }, 1000)
                            } else {
                                $.smallBox({
                                    title: response.message,
    ...
    // List Menus
    $.get("/VehicleReserves/GetReservesStatus", function (rs) {
        ...
        setTimeout(function () {
            ...
            $("#ShowStatus").on("click", ".edit", function () {
                ...
                $.get("/VehicleReserves/FormEditReservesStatus", { "ReserveStatusId": ReserveStatusId }, function (ReturnForm) {
                    ...
                    $("#submit").click(function () {
                        ...
                        if ($('#FormEditStatus').data("bootstrapValidator").isValid() == true) {
                            ...
                            {
                                type: "POST",
                                url: "/VehicleReserves/EditReserveStatus",
                                contentType: false,
                                processData: false,
                                data: Data,
                                success: function (response) {
            ...
            $("#ShowStatus").on("click", ".delete", function () {
                ...
                $.SmartMessageBox({
                    ...
                    if (ButtonPressed == "ใช่") {
                        $.get("/VehicleReserves/DeleteReserveStatus", { "ReserveStatusId": ReserveStatusId }, function (response) {
                            if (response.valid == true) {
                                $.smallBox({
                                    title: response.message,
                                    content: "<i class='fa fa-clock-o'></i> <i>" + response.message + "</i>",
