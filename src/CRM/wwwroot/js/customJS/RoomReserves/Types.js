$(function () {
    // Add Menu
    $("#add").click(function () {
        $.get("/RoomReserves/FormAddReserveType", function (ReturnForm) {
            ...
            $("#submit").click(function () {
                ...
                if ($('#FormAddType').data("bootstrapValidator").isValid() == true) {
                    ...
                    {
                        type: "POST",
                        url: "/RoomReserves/AddReserveType",
                        contentType: false,
                        processData: false,
                        data: Data,
                        success: function (response) {
                            if (response.valid == true) {
                                ...
                                setTimeout(function () {
                                    window.location.href = "/RoomReserves/ReserveTypes";
                                }, 1000)
                            } else {
                                $.smallBox({
                                    title: response.message,
    ...
    // List Menus
    $.get("/RoomReserves/GetReserveTypes", function (rs) {
        ...
        setTimeout(function () {
            ...
            $("#ShowTypes").on("click", ".edit", function () {
                ...
                $.get("/RoomReserves/FormEditReserveType", { "ReserveTypeId": ReserveTypeId }, function (ReturnForm) {
                    ...
                    $("#submit").click(function () {
                        ...
                        if ($('#FormEditType').data("bootstrapValidator").isValid() == true) {
                            ...
                            {
                                type: "POST",
                                url: "/RoomReserves/EditReserveType",
                                contentType: false,
                                processData: false,
                                data: Data,
                                success: function (response) {
            ...
            $("#ShowTypes").on("click", ".delete", function () {
                ...
                $.SmartMessageBox({
                    ...
                    if (ButtonPressed == "ใช่") {
                        $.get("/RoomReserves/DeleteReserveType", { "ReserveTypeId": ReserveTypeId }, function (response) {
                            if (response.valid == true) {
                                $.smallBox({
                                    title: response.message,
                                    content: "<i class='fa fa-clock-o'></i> <i>" + response.message + "</i>",
